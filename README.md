...
- In section ```Deploy Plugin```, click ```File Browse``` and select the ```HPI``` file:
  ```<repo>/cpln-jenkins-plugin/target/cpln-jenkins-plugin/cpln-jenkins-plugin.hpi``` 
...
- Verify that ```Control Plane Plugin Version 1.0.0``` is available and enabled;
...
Although the initial set of plugin features provides a good starting point with generic automation and utilization of Control Plane resources for some customers 
interested in leveraging Control Plane services, time and resources will need to be allocated for:
  - increasing code coverage;
  - integrating the plugin into cpln build pipelines;
  - adding integration tests;
  - opensourcing/publishing the plugin to establish and improve its adoption;
    - see https://plugin-health.jenkins.io/scores for more details on health models and criteria
  - collecting and negotiating customer requirements etc.
